...
class Application:
    ...
    def click_mouse(self, event):
        ...
        if 0 <= x < 10 and 0 <= y < 10:
            ...
            # Переключение игроков при полной расстановке
                if self.current_player.get_count() == 10:
                    if self.fl_pl2 and self.current_player == self.pl1:
                        ...
                        self.horizontal_radio=self.vertical_radio=None
                        self.switch_player()
                        self.field_pl1.delete("all")  # Очищаем поле
                    else:
    ...
    def switch_player(self):
        if self.current_player == self.pl1:
            ...
            self.field_pl1.unbind("<Button-1>")  # Отключаем клики на поле игрока 1
            self.field_pl2.bind("<Button-1>", self.click_mouse)  # Включаем клики на поле игрока 2
